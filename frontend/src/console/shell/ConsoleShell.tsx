...
export function ConsoleShell({
  ...
  useEffect(() => {
    setCollapsed(tweaks.sidebar === 'rail')
  }, [tweaks.sidebar])
  ...
  const rootClass = useMemo(
    () =>
      [
        'lattice-root',
        `lattice-theme-${tweaks.theme}`,
        `lattice-accent-${tweaks.accent}`,
        `lattice-density-${tweaks.density}`,
      ].join(' '),
  ...
  return (
    ...
    >
      <Sidebar
        ...
        collapsed={collapsed}
        onToggleCollapse={() => {
          const next = collapsed ? 'expanded' : 'rail'
          update('sidebar', next)
          setCollapsed(!collapsed)
        }}
        className="lattice-hide-sm"
      />
